...
use crate::config::icons::{RadialDashCircle, WorldMap, ArrowUp, ArrowDown};
use std::borrow::Cow;
...
#[component]
pub fn Reviews(props: ReviewsProps) -> String {
    let reviews = vec![
        // --- CLUSTER 1: EUROPE ---
        ReviewItem {
            title: "Rock-Solid Security Architecture and Outstanding Platform Reliability",
            description: "Our data integrity has remained uncompromised. The platform’s stability under peak traffic during our UK rollout proved the strength of its engineering foundation.",
        ...
        ReviewItem {
            ...
            description: "Our collaboration required extreme precision and low-level systems architecture. This solution provided the perfect framework for our hardware-software integration projects in Leuven.",
        ...
        ReviewItem {
            ...
            description: "We needed a backend capable of processing real-time telemetry from our automated systems. This solution delivered the low-latency throughput our engineers in Amsterdam demanded.",
        ...
        ReviewItem {
            ...
            description: "Finally, a framework that prioritizes clean code and efficiency. The scaling logic is robust, making our European expansion effortless and friction-free.",
        ...
        ReviewItem {
            ...
            description: "In the Swiss banking sector, security is non-negotiable. This architecture provided the perfect balance of encrypted data protection and rapid transaction processing.",
        ...
        ReviewItem {
            ...
            description: "The codebase aligns perfectly with our philosophy of efficiency and simplicity. It significantly reduced our technical debt while enhancing the end-user experience across Scandinavia.",
            image: "/images/avatars/astrid.jpg",
            name: "Astrid Jensen",
            country: "sweden",
            role: "UX Strategy Head",
        },
        // --- CLUSTER 2: THE AMERICAS ---
        ReviewItem {
            title: "Driving Canadian Tech Innovation With Cutting-Edge Cloud Solutions",
            description: "Our Canadian operations benefited from an ultra-optimized cloud infrastructure. The team’s technical rigor and proactive problem-solving accelerated project delivery and performance.",
        ...
        ReviewItem {
            ...
            description: "Their precision-focused technical SEO approach helped us secure top search positions across highly competitive U.S. markets far faster than expected.",
        ...
        ReviewItem {
            ...
            description: "Our team in Bangalore appreciated the modular design. It allowed us to iterate features rapidly and deploy updates to our global user base without downtime.",
        ...
        ReviewItem {
            ...
            description: "Integrating this solution into our Shenzhen R&D hub was seamless. The ability to handle massive concurrent user loads without latency is exactly what our ecosystem required.",
        ...
        ReviewItem {
            ...
            description: "The performance is unmatched. Our load times dropped by 60% across Seoul and Tokyo, and the fluid responsiveness feels incredibly premium on every device.",
        ...
        ReviewItem {
            ...
            description: "A masterclass in modern web design. Every interaction is smooth, precise, and perfectly aligned with the high-end aesthetic our Japanese clients demand.",
    ...
    let map_classes = "
        h-fit w-full lg:max-w-1/3 lg:h-[calc(var(--sfu)*35)] 2xl:h-[calc(var(--sfu)*42.5)]
    ...
    html! {
        <section class={["py-[calc(var(--sfu)*2)]", props.class.as_deref().unwrap_or("")].join(" ")}>
            <div data-section-content class="flex flex-col lg:flex-row items-center justify-center gap-[calc(var(--sfu)*1)] w-full">
                /* Inputs Logic */
                {
                    reviews.iter().enumerate().map(|(num, obj)| html! {
                        <input
                            key={format!("{}-rv", num)}
                            type="radio"
                            name="mindset-stack-rv"
                            id={format!("card-{}-rv", num + 1)}
                            checked={num == 0} 
                            class={format!("hidden peer/c{} peer/{}", num + 1, obj.country)}
                        />
                    }).collect::<String>()
                }

                /* Left Side: Map Panel */
                <div class={map_classes}>
                    <div class="h-full w-full rounded-full flex flex-col items-center text-center justify-between bg-[var(--color-bg-contrast)] p-[calc(var(--sfu)*1)] py-[calc(var(--sfu)*2)] mt-[calc(var(--sfu)*1)] overflow-hidden text-[var(--color-text-contrast)]">
                        <p class="leading-[1]">
                            "Working" <br /> "Worldwide"
                        </p>

                        <div class="w-full aspect-square py-[calc(var(--sfu)*2)]">
                            <div class="h-full w-full px-[calc(var(--sfu)*1)]">
                                <div class="w-full aspect-square rounded-full relative">
                                    <RadialDashCircle 
                                        class=Some("absolute inset-[calc(var(--sfu)*-0.25)] z-10 text-[var(--color-text-action)] \
                                            ease-[var(--motion-steady)]")
                                        path_class=None
                                     />
                                    <div class="w-full h-full rounded-full overflow-hidden relative">
                                        <WorldMap
                                            class=Some("wm absolute inset-0 transition-all duration-[var(--duration-extra-long)] ease-[var(--motion-steady)] \
                                                [&_path]:transition-all [&_path]:duration-[var(--duration-short)] [&_path]:ease-[var(--motion-steady)] \
                                                [&_path]:stroke-[0.2]")
                                            path_class=None
                                        />
                                    </div>
                        ...
                        <p class="font-brisa text-[calc(var(--sfu)*1.25)]">
                            "Dealing with All"
                        </p>
                    </div>
                </div>
...
#[props]
pub struct ReviewCarouselProps {
    pub data: Vec<ReviewItem>,
}

#[component]
pub fn ReviewCarousel(props: ReviewCarouselProps) -> String {
    let cards = props.data;
    
    let container_classes = r#"
        h-[calc(var(--sfu)*33)] lg:h-[calc(var(--sfu)*35)] 2xl:h-[calc(var(--sfu)*42.5)] w-full lg:w-2/3
    ...
    let mut cards_stage_html = String::new();
    for (i, p) in cards.iter().enumerate() {
        let card_class = format!("card{}-rv absolute transition-all duration-500 ease-[var(--motion-steady)]", i + 1);
        
        let card_rendered = Card(CardProps {
            name: p.name,
            title: p.title,
            image: p.image,
            role: p.role,
        ...
        cards_stage_html.push_str(&card_rendered);
    }

    html! {
        <div class={container_classes}>
            <div class="h-full w-full bg-[var(--color-bg-surface-emphasis)] mt-[calc(var(--sfu)*1)] overflow-hidden p-[calc(var(--sfu)*1)] py-[calc(var(--sfu)*3)] sm:py-[calc(var(--sfu)*1)] flex flex-col items-center justify-between relative rounded-[calc(var(--sfu)*1)]">
                
                <div />

                /* The Stage */
                <div class="relative w-full flex items-center justify-center group [perspective:1200px] [transform-style:preserve-3d] [&_>_div]:opacity-0 [&_>_div]:scale-90 [&_>_div]:pointer-events-none [&_>_div]:[backface-visibility:hidden]">
                    {cards_stage_html}
                </div>

                /* Navigation Controls */
                <div class="flex gap-[calc(var(--sfu)*0.5)] sm:mt-0 translate-y-[calc(var(--sfu)*2)] sm:translate-y-0">
                    /* Up Button */
                    <div class="p-[calc(var(--sfu)*1.25)] rounded-full bg-[var(--color-bg-contrast)] text-[var(--color-text-contrast)] relative active:scale-95 transition-transform">
                        <ArrowUp class=None path_class=None />
                        {
                            (1..=12).map(|n| html! {
                                <label for={format!("card-{}-rv", n)} class={format!("up-{}-rv absolute inset-0 hidden cursor-pointer",n)} />
                            }).collect::<String>()
                        }
                    </div>
                    /* Down Button */
                    <div class="p-[calc(var(--sfu)*1.25)] rounded-full bg-[var(--color-bg-contrast)] text-[var(--color-text-contrast)] relative active:scale-95 transition-transform">
                        ...
                        {
                            (1..=12).map(|n| html! {
                                <label for={format!("card-{}-rv", n)} class={format!("down-{}-rv absolute inset-0 hidden cursor-pointer",n)} />
                            }).collect::<String>()
                        }
                    </div>
                </div>
                ...
                <div class="hidden sm:flex flex-col gap-[calc(var(--sfu)*0.5)] absolute right-0 top-1/2 -translate-y-1/2 mr-[calc(var(--sfu)*2)]">
                    {
                        cards.iter().enumerate().map(|(i, _)| html! {
                            <div class={format!("line-{} w-[calc(var(--sfu)*0.5)] h-[calc(var(--sfu)*0.125)] rounded-full bg-[var(--color-border-emphasis)] transition-all duration-[var(--duration-medium)] ease-[var(--motion-steady)]", i + 1)} />
                        }).collect::<String>()
                    }
                </div>
            </div>
...
#[component]
fn Card(props: CardProps) -> String {
    html! {
        <div class={["w-11/12 lg:w-3/4 h-fit p-[calc(var(--sfu)*1.5)] sm:p-[calc(var(--sfu)*3)] rounded-[calc(var(--sfu)*0.75)] bg-[var(--color-electric-indigo)] flex flex-col gap-[calc(var(--sfu)*2)] text-[var(--color-text-action)]", &props.class].join(" ")}>
            <h3 class="text-[calc(var(--sfu)*1.25)] sm:text-[calc(var(--sfu)*2)] leading-tight tracking-tighter">
                {props.title}
            </h3>

            <div class="flex flex-col-reverse lg:flex-row items-start justify-between gap-[calc(var(--sfu)*1.5)]">
                <div class="flex flex-row gap-[calc(var(--sfu)*1)] lg:gap-0 lg:flex-col items-center lg:items-start justify-start">
                    <div class="w-[calc(var(--sfu)*3.25)] sm:w-[calc(var(--sfu)*4)] aspect-square rounded-full bg-[var(--color-bg-base)] relative overflow-hidden">
                        /* If using a custom image component or raw img */
                        <img src={props.image} alt={props.name} class="object-cover w-full h-full" />
                    </div>
                    <div class="flex flex-col">
                        <h3 class="font-brisa text-[calc(var(--sfu)*1.25)] pt-[calc(var(--sfu)*0.25)]">
                            {props.name}
                        </h3>
                        <h4 class="py-[calc(var(--sfu)*0.125)] px-[calc(var(--sfu)*0.25)] font-mono leading-none bg-[var(--color-bg-contrast)] text-[var(--color-text-contrast)] text-[calc(var(--sfu)*0.75)]">
                            {props.role}
                        </h4>
                    </div>
                </div>
                <p class="leading-normal font-medium opacity-90 max-w-full xl:max-w-2/3 leading-[0.95]">
                    {props.description}
                </p>
            </div>
        </div>
